...
public class WebSocketEnvelope {
    ...
    TextSecure.Envelope envelope;

    public WebSocketEnvelope(byte[] ciphertext, String signalingKey) throws IOException {
        SecretKeySpec cipherKey  = getCipherKey(signalingKey);
        SecretKeySpec macKey     = getMacKey(signalingKey);
        verifyMac(ciphertext, macKey);
        envelope = TextSecure.Envelope.parseFrom(getPlaintext(ciphertext, cipherKey));
    }

    public TextSecure.Envelope get(){
    ...
    private void verifyMac(byte[] ciphertext, SecretKeySpec macKey) throws IOException {
        try {
            ...
            Timber.w("Our MAC: " + Hex.toString(ourMacBytes));
            Timber.w("Thr MAC: " + Hex.toString(theirMacBytes));
