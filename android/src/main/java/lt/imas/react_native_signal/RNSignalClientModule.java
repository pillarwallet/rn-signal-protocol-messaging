...
public class RNSignalClientModule extends ReactContextBaseJavaModule {
    ...
    public void addContact(ReadableMap params, boolean forceAdd, Promise promise){
        try {
            ...
            if (!protocolStorage.containsSession(address) || forceAdd){
                signalClient.requestPreKeys(username, params.getString("userId"), params.getString("targetUserId"), promise);
            } else {
                promise.resolve("ok");
            }
        } catch (Throwable e) {
    ...
    public void sendSilentMessageByContact(String tag, ReadableMap params, final Promise promise) {
        try {
            signalClient.sendMessage(params.getString("username"), params.getString("message"), params.getString("userId"), params.getString("targetUserId"), tag, true, promise);
        } catch (Throwable e) {
            logSender.reportError(e);
            promise.reject(e);
        }
    ...
    public void sendMessageByContact(String tag, ReadableMap params, final Promise promise) {
        try {
            signalClient.sendMessage(params.getString("username"), params.getString("message"), params.getString("userId"), params.getString("targetUserId"), tag, false, promise);
        } catch (Throwable e) {
            logSender.reportError(e);
            promise.reject(e);
        }
    ...
    public void prepareApiBody(String tag, ReadableMap params, final Promise promise) {
        try {
            JSONObject request = signalClient.prepareApiBody(params.getString("username"), params.getString("message"), params.getString("userId"), params.getString("targetUserId"), tag, false);
