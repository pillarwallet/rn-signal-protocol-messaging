...
class SignalClient: NSObject {
  ...
  func register(completion: @escaping (_ success: Bool) -> Void) {
    ...
    self.signalServer.call(urlPath: URL_ACCOUNTS, method: .PUT, parameters: parameters, success: { (dict) in
      do {
        try self.registerPreKeys(store: store, completion: { (success) in
          completion(success)
        })
      } catch {
        completion(false)
  ...
  private func registerPreKeys(store: SignalStore, completion: @escaping (_ success: Bool) -> Void) throws {
  ...
  func requestPreKeys(username: String, completion: @escaping (_ success: Bool) -> Void) {
  ...
  func getContactMessages(username: String, decodeAndSave: Bool, completion: @escaping (_ success: Bool) -> Void) {
  ...
  func sendMessage(username: String, messageString: String, completion: @escaping (_ success: Bool) -> Void) {
    guard let store = self.store() else {
      completion(false)
      print(ERR_NATIVE_FAILED)
      return
